...
class LinkNodeProviderTest {

    var titleRetriever: TitleRetriever = mock(TitleRetriever::class.java)
    var finder: LinkFinder = mock(LinkFinder::class.java)

    lateinit var linkNodeProvider: LinkNodeProvider

    @Before
    fun setUp() {
        linkNodeProvider = LinkNodeProvider(finder, titleRetriever)
    }
    ...
    fun simpleTest() {
        ...
        val single = linkNodeProvider.from(input)
